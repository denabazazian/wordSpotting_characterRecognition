# -*- coding: utf-8 -*-
"""
Created on Wed Feb 22 16:12:35 2017
...
#net = caffe.Net('/Projects/caffe_CharacterDetection/synth-fcn8s-atonce/deploy.prototxt', '/Projects/caffe_CharacterDetection/snapshot/train_iter_3530000.caffemodel', caffe.TEST) # net that has NAN as the exponentional scores 
...
net = caffe.Net('/Projects/caffe_CharacterDetection/synth-fcn8s-atonce/deploy.prototxt', '/Projects/caffe_CharacterDetection/snapshot/train_iter_4620000.caffemodel', caffe.TEST)
...
for idx in range(1,2):
    ...
    im = Image.open('/Projects/CharacterDetection/HeatMaps/cropped/img_844_cropped_2.jpg')
    print idx
    ...
    # Heatmap computation
    net_scores = net.blobs['score_conv'].data[0][:, :, :]
    ...
    plt.imshow(heatMap[:,:,1+ ord('a')-ord('a')]);plt.imshow(im,alpha=.5);plt.title('a-%d'% (1+ ord('a')-ord('a'))); plt.show()
